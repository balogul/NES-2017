...
:: Path names
@set DEST_PATH=app\FlashOnly_OAD_ExtFlash
@set APP_PATH=app\FlashOnly_OAD_ExtFlash
@set STACK_PATH=stack\FlashROM
@set BIM_PATH=..\..\..\util\bim_extflash\cc1350\ccs\FlashOnly_LP

:: Set application name
@set APP_NAME=simple_peripheral_cc1350lp
...
:: 2) Full executable image (application + stack + BIM). For use with flash programmer or OAD
"C:\Python27\python" "C:\\Python27\\Scripts\hexmerge.py" -o "%DEST_PATH%\%ALL_IMG%" "--overlap=error"  "%DEST_PATH%\%APST_IMG%" %BIM_PATH%\bim_extflash_cc1350lp_bim.hex
